...
(function () {
     ...
     *********************************************************/
    let BAD_RESTAURANT_ACTION: BadResultAction = BadResultAction.MoveAndFade;
    ...
    interface HygieneRating {
        ...
        rating: number,
        localCacheDate: number
    }

    class RestaurantManager {

        url: string = "https://hygieneratingscdn.je-apis.com/api/uk/restaurants/";
        cacheTime: number;
        ...
        getRestaurant(key: string): HygieneRating {
            let data: HygieneRating | null = JSON.parse(window.localStorage.getItem(key));
        ...
        private fetchData(key: string): HygieneRating {
            let url = this.url + key;
            let request = new XMLHttpRequest();
            request.open('GET', url, false);
            request.send(null);
            let data = <HygieneRating>JSON.parse(request.responseText);
    ...
    const restaurants: NodeListOf<HTMLElement> = document.querySelectorAll('section[data-restaurant-id]');
    let manager = new RestaurantManager(CACHE_TIME);

    restaurants.forEach(function (element: HTMLElement) {
